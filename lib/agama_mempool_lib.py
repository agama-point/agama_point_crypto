...
class Mempool():
    ...
    def get_block_info(self, b, debug2 = False, tx_info = True):
        ...
        txs_list =  json.loads(json_data_txs)

        lentx = len(txs_list)
        print(f"[{b}] {block_hash} | {dt}  ({lentx})")
        ...
        if lentx > 0:
            ...
            if tx_info:
                for tx_id in txs_list:
                    #print(self.get_tx_info(tx_id, debug2 = self.debug))
    ...
    def get_tx_info(self, tx_id, debug2 = False):
        ...
        if len(tx_id) != 64:
            raise ValueError(url, "Invalid transaction ID")
